...
type AvailabilityKey = "hoje" | "amanha" | "semana" | "quinze" | "indisponivel";
...
function filterWorkers(workers: Worker[], form: FormState): Worker[] {
  return workers.filter((w) => {
    ...
    if (form.specialty && !w.tags.includes(form.specialty)) return false;
...
export default function DemandPage() {
  ...
  const setSpecialty = (value: string) =>
    setForm((f) => ({ ...f, specialty: f.specialty === value ? "" : value }));

  const setMaxKm = (km: number | null) =>
    setForm((f) => ({ ...f, maxKm: f.maxKm === km ? null : km }));
  ...
  const setMinRating = (rating: number | null) =>
    setForm((f) => ({ ...f, minRating: f.minRating === rating ? null : rating }));
          ...
          >
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "24px" }}>
                <div style={{ width: "10px", height: "40px", borderRadius: "5px", backgroundColor: "#E0C271", flexShrink: 0 }} />
                <p style={{ fontFamily: "'SF Pro Text', system-ui, sans-serif", fontWeight: 600, fontSize: "36px", color: "#FAF9F5", margin: 0 }}>
                  Localização — até quantos km de mim?{" "}
                  <span style={{ fontWeight: 300, fontSize: "28px", color: "#888" }}>— opcional</span>
                </p>
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "14px" }}>
                {[
                  ...
                  <Chip
                    ...
                    selected={
                      value === null
                        ? form.maxKm === null && form.profession !== "" // "Qualquer" is selected when maxKm is null and form was interacted
                        : form.maxKm === value
                    }
                    onToggle={() => setMaxKm(value)}
                  />
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: "50px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "14px" }}>
                {[
                  ...
                  { label: "★ 4,8 ou mais", value: 4.8 },
                  { label: "★ 4,9 ou mais", value: 4.9 },
                  { label: "★ 5,0 exato", value: 5.0 },
                ].map(({ label, value }) => (
                  <Chip
                    key={label}
                    label={label}
                    selected={form.minRating === value}
                    onToggle={() => setMinRating(value)}
                  />
                ))}
              </div>
            </div>
