...
function buildRealOverviewSeries(
  ...
  for (let i = 0; i < points; i++) {
    ...
    const gross = curr.filter((t) => isRecebimentoPrestador(t)).reduce((s, t) => s + parseFloat(t.valor), 0);
    ...
    currentCustomers.push(curr.filter((t) => isRecebimentoPrestador(t)).length);
    ...
    const cGross = comp.filter((t) => isRecebimentoPrestador(t)).reduce((s, t) => s + parseFloat(t.valor), 0);
    ...
    comparisonCustomers.push(comp.filter((t) => isRecebimentoPrestador(t)).length);
  }
  ...
  const makeHourSeries = (base: Date) =>
    Array.from({ length: 24 }, (_, h) => {
      ...
      return transacoes
        .filter((t) => {
          const d = new Date(t.created_at);
          return d >= s && d < e && isRecebimentoPrestador(t);
        })
