  ...
  it("deve lançar ValidationError para UUID inválido", async () => {
    ...
    await expect(
      sut.executar({
        user_id: "id-invalido",
        nome: "João",
        bio: "Bio válida com texto suficiente",
      }),
...
describe("DeletarPrestadorUseCase", () => {
  it("deve deletar um prestador existente", async () => {
    const prestadorRepo = makePrestadorRepo({
      findByUserId: jest.fn().mockResolvedValue(PRESTADOR_MOCK),
    });
...
describe("AtualizarPrestadorUseCase", () => {
  it("deve atualizar nome e bio de um prestador existente", async () => {
    const prestadorExistente = new Prestador({
      user_id: UUID_VALIDO,
      nome: "Nome Antigo",
      bio: "Bio antiga mas válida aqui",
    });
    const prestadorRepo = makePrestadorRepo({
