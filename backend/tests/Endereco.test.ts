...
const USER_MOCK     = new User({ id: UUID_VALIDO, email: 'u@t.com', senha: 'hash', cpf: '12345678900' });
...
describe('BuscarCepUseCase', () => {
  ...
  it('deve retornar os dados do endereço para um CEP válido', async () => {
    ...
    expect(resultado).toMatchObject({ logradouro: 'Avenida Paulista', localidade: 'São Paulo' });
  });
  ...
  it('deve aceitar CEP com máscara e sanitizar antes de consultar', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('01310100'));
...
describe('CriarEnderecoUseCase', () => {
  ...
  it('deve lançar ValidationError para UUID inválido', async () => {
    ...
    await expect(sut.executar({ ...DADOS_ENDERECO, user_id: 'id-invalido' }))
  ...
  it('deve lançar ValidationError para CEP com dígitos insuficientes', async () => {
    ...
    await expect(sut.executar({ ...DADOS_ENDERECO, cep: '1234' }))
      .rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it('deve lançar ValidationError para logradouro com menos de 3 caracteres', async () => {
    ...
    await expect(sut.executar({ ...DADOS_ENDERECO, logradouro: 'Av' }))
      .rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it('deve lançar ValidationError para cidade com menos de 2 caracteres', async () => {
    ...
    await expect(sut.executar({ ...DADOS_ENDERECO, cidade: 'X' }))
      .rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it('deve lançar ResourceNotFoundError quando o usuário não existe', async () => {
    ...
    await expect(sut.executar(DADOS_ENDERECO)).rejects.toBeInstanceOf(ResourceNotFoundError);
  });
});
...
describe('DeletarEnderecoUseCase', () => {
  ...
  it('deve lançar ValidationError para UUID inválido', async () => {
    ...
    await expect(sut.executar('invalido')).rejects.toBeInstanceOf(ValidationError);
  });
});
...
describe('AtualizarEnderecoUseCase', () => {
  it('deve atualizar o endereço quando encontrado', async () => {
    const enderecoRepo = makeEnderecoRepo({ findById: jest.fn().mockResolvedValue(ENDERECO_MOCK) });
    const sut = new AtualizarEnderecoUseCase(enderecoRepo);

    const resultado = await sut.executar({ id: UUID_ENDERECO, cidade: 'Campinas', is_principal: false });
  ...
  it('deve lançar ResourceNotFoundError quando o endereço não existe', async () => {
    ...
    await expect(
      sut.executar({ id: UUID_ENDERECO, cidade: 'Campinas', is_principal: false }),
...
describe('AcharPorUserId', () => {
  it('deve retornar o endereço do usuário quando encontrado', async () => {
    const enderecoRepo = makeEnderecoRepo({ findByUserId: jest.fn().mockResolvedValue(ENDERECO_MOCK) });
...
describe('AcharPorPrestadorId', () => {
  it('deve retornar o endereço do prestador quando encontrado', async () => {
    const enderecoRepo = makeEnderecoRepo({ findByPrestadorId: jest.fn().mockResolvedValue(ENDERECO_MOCK) });
  ...
  it('deve lançar ResourceNotFoundError quando não existe endereço para o prestador', async () => {
    ...
    await expect(sut.executar(UUID_PRESTADOR)).rejects.toBeInstanceOf(ResourceNotFoundError);
  });
});
...
describe('AcharPorCidade', () => {
  it('deve retornar endereços de uma cidade', async () => {
    const enderecoRepo = makeEnderecoRepo({ findByCity: jest.fn().mockResolvedValue([ENDERECO_MOCK]) });
...
describe('CalcularDistanciaUseCase', () => {
  const END_USUARIO   = new Endereco({ ...DADOS_ENDERECO, cep: '01310100' }, UUID_ENDERECO);
  const END_PRESTADOR = new Endereco({ ...DADOS_ENDERECO, user_id: UUID_PRESTADOR, cep: '20040020' }, UUID_PRESTADOR);

  const VIACEP_SP = { logradouro: 'Av Paulista', bairro: 'Bela Vista', localidade: 'São Paulo', uf: 'SP' };
  const VIACEP_RJ = { logradouro: 'Av Rio Branco', bairro: 'Centro', localidade: 'Rio de Janeiro', uf: 'RJ' };
  ...
  function mockFetchPorUrl({
    ...
    nominatimFull = (url: string) => url.includes('Paulista') || url.includes('Branco') ? [] : [],
    ...
    (global.fetch as jest.Mock).mockImplementation(async (url: string) => {
      if (url.includes('viacep.com.br/ws/01310100')) return { ok: true, json: async () => viacepUser };
      if (url.includes('viacep.com.br/ws/20040020')) return { ok: true, json: async () => viacepPrestador };
      if (url.includes('nominatim')) {
        // Heurística: se a URL contém o logradouro, é a query completa; senão é a query por cidade
        if (url.includes('Paulista') || url.includes('Bela')) return { ok: true, json: async () => coordsUser };
        if (url.includes('Branco')   || url.includes('Centro')) return { ok: true, json: async () => coordsPrestador };
  ...
  it('deve lançar ValidationError para UUID de usuário inválido', async () => {
    ...
    await expect(sut.executar('invalido', UUID_PRESTADOR)).rejects.toBeInstanceOf(ValidationError);
  });

  it('deve lançar ValidationError para UUID de prestador inválido', async () => {
    const sut = new CalcularDistanciaUseCase(makeEnderecoRepo());

    await expect(sut.executar(UUID_VALIDO, 'invalido')).rejects.toBeInstanceOf(ValidationError);
  });

  it('deve lançar ResourceNotFoundError quando endereço do usuário não existe', async () => {
    const enderecoRepo = makeEnderecoRepo({ findByUserId: jest.fn().mockResolvedValue(null) });
    const sut = new CalcularDistanciaUseCase(enderecoRepo);

    await expect(sut.executar(UUID_VALIDO, UUID_PRESTADOR)).rejects.toBeInstanceOf(ResourceNotFoundError);
  });
  ...
  it('deve lançar ValidationError quando o ViaCEP retorna erro para ambos os CEPs', async () => {
    (global.fetch as jest.Mock).mockImplementation(async (url: string) => {
      if (url.includes('viacep.com.br')) return { ok: true, json: async () => ({ erro: true }) };
      return { ok: true, json: async () => [] };
    });
    const enderecoRepo = makeEnderecoRepo({
      findByUserId:      jest.fn().mockResolvedValue(END_USUARIO),
    ...
    await expect(sut.executar(UUID_VALIDO, UUID_PRESTADOR)).rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it('deve usar fallback por cidade quando Nominatim não encontra o endereço completo', async () => {
    ...
    (global.fetch as jest.Mock).mockImplementation(async (url: string) => {
      if (url.includes('viacep.com.br/ws/01310100')) return { ok: true, json: async () => VIACEP_SP };
      if (url.includes('viacep.com.br/ws/20040020')) return { ok: true, json: async () => VIACEP_RJ };
      // Nominatim: query completa (tem logradouro) → vazio; query cidade (só localidade) → coords
      if (url.includes('Paulista') || url.includes('Branco')) return { ok: true, json: async () => [] };
      if (url.includes('nominatim')) return { ok: true, json: async () => COORDS_SP };
      return { ok: true, json: async () => [] };
    });
    const enderecoRepo = makeEnderecoRepo({
      findByUserId:      jest.fn().mockResolvedValue(END_USUARIO),
