...
export class AtualizarCategoriaUseCase {
  ...
  async executar(dados: AtualizarCategoriaDto) {
    ...
    if (dados.nome) { validarTexto(dados.nome, 'Nome', 2, 60); dados.nome = sanitizarTexto(dados.nome); }
    if (dados.slug) { validarTexto(dados.slug, 'Slug', 2, 60); dados.slug = sanitizarTexto(dados.slug); }
